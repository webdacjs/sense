(function () {
  ...
  function getAutoCompleteContext (editor) {
    ...
    if (!context.autoCompleteSet || !context.autoCompleteSet.completionTerms ||
            context.autoCompleteSet.completionTerms.length == 0) { return null } // nothing to do..
  ...
  function addBodyPrefixSuffixToContext (context, editor) {
    ...
    if (context.replacingToken) {
      ...
      var pos = editor.getCursorPosition()
      if (pos.column == context.updatedForToken.start) { insertingRelativeToToken = -1 } else if (pos.column < context.updatedForToken.start + context.updatedForToken.value.length) { insertingRelativeToToken = 0 } else {
    ...
    switch (nonEmptyToken ? nonEmptyToken.type : 'NOTOKEN') {
      ...
      case 'method':
        break
      default:
        if (!nonEmptyToken || !nonEmptyToken.type.indexOf('url') == 0) { context.prefixToAdd = ', ' }
    }
  ...
  function addBodyAutoCompleteSetToContext (context, editor) {
    ...
    function getLinkedRules (link, currentRules) {
      ...
      if (scheme_id == 'GLOBAL') {
        ...
        if (!linked_rules) { throw 'No autocomplete rules defined in linked scheme: ' + scheme_id }
      }
    ...
    function extractOptionsForPath (rules, tokenPath, activeScheme) {
      ...
      if (rules) {
        if (typeof rules === 'string') {
          ...
          if (rules.__any_of.length > 0 && typeof rules.__any_of[0] !== 'object') { $.merge(autocompleteSet.completionTerms, rules.__any_of) }
        } else if (typeof rules === 'object') {
          for (term in rules) {
            if (typeof term === 'string' && term.match(/^__|^\*$/)) { continue } // meta term
            ...
            if (typeof rules_for_term.__template !== 'undefined') { autocompleteSet.templateByTerm[term] = rules_for_term.__template } else if (rules_for_term instanceof Array) {
              var template = []
              ...
              if (rules_for_term.__one_of) { autocompleteSet.templateByTerm[term] = rules_for_term.__one_of[0] } else if ($.isEmptyObject(rules_for_term))
